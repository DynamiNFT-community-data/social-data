...
tweets_df = pd.DataFrame(tweets, columns=['date', 'user', 'username','content', 'tweet_id', 'url', 'replies', 'retweets','likes', 'quotes', 'mentioned_users', 'lang', 'retweet_source', 'quoted_source', 'media', 'outlinks', 'tcooutlinks'])
...
tweets_df_agg['contributors'] = tweets_df_unique_users.resample(resample_interval, on = 'date', origin = 'start_day')['count'].sum()
...
for row in tweets_df_unique_users.user:
    twitter_pfps = twitter_pfps.append({'user_id': row.id, 'profile_image_url': row.profileImageUrl.replace("_normal", '')  }, ignore_index = True)
